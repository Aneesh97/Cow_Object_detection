...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img0 = cv2.imread(path, cv2.IMREAD_UNCHANGED)  # BGR/BGRA
        ...
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR/BGRA to RGB/ARGB, to 3x416x416
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_labels=True, cache_images=False, single_cls=False, cv2_read_flag=cv2.IMREAD_COLOR):
        path = str(Path(path))  # os-agnostic
        ...
        if cache_images:  # if training
            ...
            for i in pbar:  # max 10k images
                self.imgs[i], self.img_hw0[i], self.img_hw[i] = load_image(self, i, self.cv2_read_flag)  # img, hw_original, hw_resized
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w) = load_image(self, index, self.cv2_read_flag)
...
def load_image(self, index, cv2_read_flag=cv2.IMREAD_COLOR):
    # loads 1 image from dataset, returns img, original hw, resized hw
    img = self.imgs[index]
    if img is None:  # not cached
        img_path = self.img_files[index]
        img = cv2.imread(img_path, cv2_read_flag)  # BGR
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w) = load_image(self, index, self.cv2_read_flag)
