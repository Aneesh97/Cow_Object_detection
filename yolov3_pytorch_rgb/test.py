...
#     iouv = iouv[0].view(1)  # comment for mAP@0.5:0.95
    ...
    seen = 0
    model.eval()
    _ = model(torch.zeros((1, nchannels, img_size, img_size), device=device)) if device.type != 'cpu' else None  # run once
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        f = os.path.join(save_path, 'test_batch%g.png' % batch_i)  # filename
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/RGB.data', help='*.data path')
    parser.add_argument('--weights', type=str, default='RGB/backup/weights/best_RGB.pt', help='weights path')
    ...
    parser.add_argument('--map_avg', action='store_true', help='calculate map@0.5:0.95')
