...
def train():
    ...
    with open(os.path.join(opt.save_path, 'opt.txt'), 'w') as f:
    ...
    cv2_read_flag = cv2.IMREAD_UNCHANGED if nchannels==4 else cv2.IMREAD_COLOR
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni < 1:
                f = os.path.join(save_path, 'train_batch%g.png' % i)  # filename
                plot_images(imgs=imgs, targets=targets, paths=paths, fname=f)
                if tb_writer:
                    tb_writer.add_image(f, cv2.imread(f, cv2_read_flag)[:, :, ::-1], dataformats='HWC')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--augment', action='store_true', help='resume training from last.pt')
